...
def FindRunout():
    ...
    return filtered_points_mm, max_deviation_point_mm, max_deviation_point_pixel, orig_image
...
def RotateEndoscope(Angle,Direction):
    ...
    arduino.write(dataStr.encode()) #This line sends the data to the arduino as a string 
    print(dataStr)
    time.sleep(2)
    print("done")
...
def MakeUI(filtered_points_mm,max_deviation_point_mm,max_deviation_point_pixel,orig_image):
    root = tk.Tk()
...
def FindRunoutAndRotateEndoscope(Readings, Angle):
    for reads in range(0,Readings):
        reads = reads+1
        Filmm, Maxdmm, maxdpp, orgimg = FindRunout()
        MakeUI(Filmm, Maxdmm, maxdpp, orgimg)
        if reads < Readings: 
            RotateEndoscope(Angle,1)
        print (reads)
