...
MANUAL_ROI_START_X = 430  # X coordinate where the ROI starts
MANUAL_ROI_START_Y = 900  # Y coordinate where the ROI starts
MANUAL_ROI_WIDTH = 3000    # The width of the ROI
...
def detect_endoscope_nozzle(image, blur_value, block_size, C, Dila, ROI_X, ROI_Y, ROI_WIDTH, ROI_HEIGHT):
    ...
    def find_intersection(x, start_y, selected_contour=None):
        y = start_y
        while 0 <= y < image.shape[0]:
            if selected_contour is not None:
                if cv2.pointPolygonTest(selected_contour, (x, y), False) >= 0:
                    return x, y
            elif any(cv2.pointPolygonTest(contour, (x, y), False) >= 0 for contour in contours):
                return x, y
            y += 1
        return None
    ...
    for x in range(ROI_X, ROI_X + ROI_WIDTH, vertical_spacing):
        intersection = find_intersection(x, ROI_Y, selected_contour)

        if intersection:
            cv2.line(image, (x, ROI_Y), intersection, (0, 255, 255), 1)

            # Set first_intersection if it's not already set
            if first_intersection is None:
                first_intersection = intersection
                selected_contour = next(
                    (contour for contour in contours if cv2.pointPolygonTest(contour, intersection, False) >= 0), None)

            # Ensure first_intersection is set before computing deviation
            if first_intersection:
                deviation_y = intersection[1] - first_intersection[1]
                if abs(deviation_y) > 2:
                    cv2.circle(image, intersection, 3, (0, 0, 255), -1)  # Mark significant deviation points in red
                # Mark the first intersection point
                cv2.circle(image, first_intersection, 5, (255, 0, 0), -1)  # Red circle
    # Draw reference line based on the first intersection
    if first_intersection:
