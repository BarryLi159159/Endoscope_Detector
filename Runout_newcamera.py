...
MANUAL_ROI_START_X = 75  # X coordinate where the ROI starts
MANUAL_ROI_START_Y = 250  # Y coordinate where the ROI starts
MANUAL_ROI_WIDTH = 800  # The width of the ROI
MANUAL_ROI_HEIGHT = 90  # The height of the ROI
...
PPMMw = calibration_data['PPMMx'] / 4
PPMMh = calibration_data['PPMMy'] / 4
...
def detect_endoscope_nozzle(image, blur_value, block_size, C, Dila, ROI_X, ROI_Y, ROI_WIDTH, ROI_HEIGHT):
    ...
    dilated = cv2.dilate(masked_thresh, kernel, Dila)
...
if max_deviation_point_mm[1] > 0.5:
    print("The endoscope is bent")
elif max_deviation_point_mm[1] <= 0.5:
...
if max_deviation_point_mm:
    deviation_text = f"Deviation (mm): {abs(max_deviation_point_mm[1])}\nDeviation (pixels): {max_deviation_point_pixel[1]}"
